...
export const SetCard = ({ actId, set }: Props) => {
  ...
  const { error: promoteError, mutate, isPending } = useMutation({ mutationFn: promoteSet });

  if (!set) return <p>Loading Set...</p>;
  ...
  return (
    <div>
      ...
      <div className="flex gap-10 bg-blue-500 p-10">
        ...
        <button className="cursor-pointer disabled:cursor-wait" type="button" disabled={isPending} onClick={() => mutate(set.id)}>Use</button>
