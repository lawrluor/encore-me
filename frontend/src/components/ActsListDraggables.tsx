    ...
    <div className="rounded-md">
      {acts?.map((act: Act, index: number) => {
        ...
        return <div key={act.id} className="relative">
          ...
          {isSelected && <Link href={`/act/${act.id}/edit`} className="absolute right-10 top-10 text-foreground-muted hover:opacity-60 transition-all duration-[0.15s] ease-in"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-pencil-icon lucide-pencil h-16 w-16"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" /><path d="m15 5 4 4" /></svg></Link>}
        </div>
      })}
    </div>
  )
