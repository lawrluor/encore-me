...
export const SetPanel = async ({ songs, actId, set, isCreateMode = false }: Props) => {
  ...
  return (
    <div className={`p-20 bg-surface ${isCreateMode ? 'rounded-b-md' : 'rounded-md'}`}>
      <header>
        <form action={isCreateMode ? postSetAction : updateSetAction.bind(null, set.id, actId)} className="flex justify-between">
          {isCreateMode && <input type="hidden" name="actId" value={actId} />}
          <div>
