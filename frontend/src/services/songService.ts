...
export const getSongs = async (idType: 'actId' | 'setId', id: string): Promise<Song[]> => {
  // By default, doesn't have authorization because fetching from server and doesn't have client auth headers
  // We cannot include cookies in header with credentials: 'include', that only works from client side
  // So, we need to get cookie another way: using cookies() from next/headers that allows us to view cookies on the server
